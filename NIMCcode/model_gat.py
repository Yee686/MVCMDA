...
class Model(nn.Module):
    def __init__(self, sizes):
        ...
        self.gat_x1 = conv.GATConv(self.fg, self.fg, heads = self.ha, add_self_loops = False)
        self.dropout_x1 = nn.Dropout(0.2)
        self.gat_x2 = conv.GATConv(self.fg*self.ha, self.fg, heads = self.hb, add_self_loops = False)
        self.dropout_x2 = nn.Dropout(0.2)
        
        self.gat_y1 = conv.GATConv(self.fd, self.fd, heads = self.ha, add_self_loops = False)
        self.dropout_y1 = nn.Dropout(0.2)
        self.gat_y2 = conv.GATConv(self.fd*self.ha, self.fd, heads = self.hb, add_self_loops = False)
        self.dropout_y2 = nn.Dropout(0.2)

        self.linear_x_1 = nn.Linear(self.fg*self.hb, 256)
        self.linear_x_2 = nn.Linear(256, 128)
        self.linear_x_3 = nn.Linear(128, self.k)

        self.linear_y_1 = nn.Linear(self.fd*self.hb, 256)
